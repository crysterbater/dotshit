...
define(['jquery', 'underscore', 'backbone', '/bext/pub/backbone.js',
    ...
    '/bext/vpn/pub/defines.js', '/util/user_agent.js',
    '/bext/vpn/pub/agent.js', '/util/util.js', '/bext/vpn/pub/pac.js',
    '/util/array.js', '/util/attrib.js', '/util/string.js',
    '/svc/pub/unblocker_lib.js', '/bext/vpn/pub/hybrid_mock.js'],
    function($, _, Backbone, be_backbone, etask, be_util, be_tabs, be_ext,
    pac_engine, B, svc_util, version_util, zescape, be_lib, zurl,
    date, zerr, browser, be_vpn_util, be_defines, user_agent, be_agent, zutil,
    be_pac, array, attrib, string, unblocker_lib, hybrid_mock){
...
function _on_before_request(details, host, route_str, opt){
    ...
    if (!chrome)
    {
        ...
        E.ff_proxy_for_url[url] = assign({route_opt: route_opt,
            t: ff_proxy_timer(url)}, {});
        return;
    }
    ...
    if (req.hola_routing)
    {
        ...
        case 'direct_first':
            ...
            else if (req.hdrs_sent && req.hdrs_recv && req.direct_resp &&
    ...
    if (req.redirected)
        return;
    be_pac.set_proxy_for_url(details.url, proxy_str);
    // XXX maximk: for some reason pac file is accessed before this function
    if (hybrid_mock.initialized && !req.redirected)
    {
        E.requests[details.requestId] = req;
        req.redirect_route_str = route_str;
        req.redirected = true;
        details.ret.redirectUrl = req.url;
    }
}
...
function tab_unblocker_cb(details){
    ...
    if (req = E.internal_reqs[url])
    {
        ...
        if (!chrome || req.reqid==details.requestId)
        {
            ...
            route_str = E.gen_route_str(assign({},
...
function build_proxy_string(req, port){
    ...
    var use_ip = chrome || version_util.cmp(be_util.version(), '1.4.151')>=0;
...
function be_tabs_add_listener(tab_unblocker, tabid){
    ...
    if (b.browser=='chrome' && +b.version>=23)
        extra_opt.push('requestBody');
    if (b.browser=='firefox' && +b.version>=53)
        extra_opt.push('requestBody');
...
E.setup_cookies_cleanup = function(rule){
    E.cookies_cleanup_urls.push(assign({}, rule)); };
...
E.update_rule_urls = function(rules){
    ...
    return etask([function(){
        ...
        for (var r in rules.unblocker_rules)
        {
            ...
            if (rule.peer)
            {
                agent_spec = E.gen_route_str({country: rule.country,
                    peer: false}).toLowerCase();
                be_agent.agents[agent_spec] = be_agent.agents[agent_spec]||[];
            }
        }
...
E.init = function(){
    ...
    has_res_hdrs_url_redir = !chrome ||
        +user_agent.guess_browser().version>=35;
    tab_listeners = {};
    tab_listeners_n = 0;
    E.url_to_rule_infos = {};
