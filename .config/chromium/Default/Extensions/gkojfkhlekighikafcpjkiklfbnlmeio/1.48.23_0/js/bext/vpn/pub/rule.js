...
E.set_rule = function(opt){
    if (be_commercial.subscribe_if_not_allowed(opt))
        return false;
...
function _verify_proxy(opt){
    // XXX maximk: disable in ff ext until it works
...
E.change_proxy = function(opt){
    ...
    return etask({name: 'change_proxy', cancel: true}, [function(){
        ...
        be_commercial.track_usage(opt, true);
